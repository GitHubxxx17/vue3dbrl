...
//头像裁剪插件
export default (can_obj,img_obj,canvas_div) => {
    console.log(can_obj,img_obj,canvas_div)
    //设置图片自适应大小及图片的居中显示
    ...
    let capture_X = can_obj.offsetLeft;
    let capture_Y = can_obj.offsetTop;
    let capture_W = can_obj.offsetWidth;
    let capture_H = can_obj.offsetHeight;
    //记录在缩放动作执行前的 缩放值
    var scale = 0;
    //开始缩放比例
    var start_sqrt = 0;
